    ...
    <%@page import="java.util.ArrayList" %>
        <%@page import="myPackage.DatabaseClass" %>
            ...
            <%@ page isELIgnored="true" %>
                ...
                <% myPackage.DatabaseClass pDAO=myPackage.DatabaseClass.getInstance(); // Get course names for dropdown
                    ...
                    courseNames = new ArrayList<>();
                        ...
                        <div id="validationModal" class="modal" style="display: none;">
                            <div class="modal-content">
                                <div class="modal-header">
                                    <h3 id="modalTitle"><i class="fas fa-exclamation-triangle"></i> Validation Error
                                    </h3>
                        ...
                        <div id="aiConfirmationModal" class="modal" style="display: none;">
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p id="aiModalMessage">We detected pasted study material. Would you like to
                                        auto-generate questions and answers from this content?</p>
                                </div>
                                <div class="modal-footer">
                                    <button onclick="confirmAIGeneration()" class="btn btn-success"
                                        id="confirmAIBtn">Generate Questions</button>
                                    <button onclick="closeAIModal()" class="btn btn-outline">Cancel (Manual
                                        Entry)</button>
                                </div>
                            </div>
                        </div>
                        ...
                        <style>
                            ...
                            * {
                            ...
                            @keyframes pulseHighlight {
                                0% {
                                    box-shadow: 0 0 0 0 rgba(9, 41, 77, 0.7);
                                }

                                70% {
                                    box-shadow: 0 0 0 10px rgba(9, 41, 77, 0);
                                }

                                100% {
                                    box-shadow: 0 0 0 0 rgba(9, 41, 77, 0);
                                }
                            }
                                        ...
                                        <form id="pdfUploadForm" enctype="multipart/form-data">
                                            <div class="form-grid">
                                                <div class="form-group">
                                                    ...
                                                    <select name="coursename" class="form-select" id="courseSelectPdf"
                                                        required>
                                                        <% if (courseNames.isEmpty()) { %>
                                                            <option value="">No courses available</option>
                                                            <% } else { %>
                                                                <option value="">Select Course</option>
                                                                <% for (String course : courseNames) { boolean
                                                                    isSelected=(lastCourseName !=null &&
                                                                    lastCourseName.equals(course)); %>
                                                                    <option value="<%=course%>" <%=isSelected
                                                                        ? "selected" : "" %>><%=course%>
                                                                    </option>
                                                                    <% } } %>
                                                    </select>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <input type="number" id="numQuestionsPdf" class="form-control"
                                                        placeholder="Auto-detect based on length" min="1" max="50">
                                                </div>
                                            </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <div class="drop-zone" id="dropZone">
                                                    <div class="drop-zone-content">
                                                        ...
                                                        <p class="drop-text">Drag & drop your PDF here or click to
                                                            browse</p>
                                                        <p class="drop-hint">Supports PDF (Max 5MB)</p>
                                                        <input type="file" name="pdfFile" class="form-control"
                                                            id="pdfFile" accept=".pdf" style="display: none;">
                                                    </div>
                                                </div>
                                                ...
                                                <small class="form-hint">Upload a PDF file to auto-generate questions
                                                    using AI</small>
                                            </div>
                                ...
                                <div class="question-card" id="addQuestionPanel">
                                    ...
                                    <div class="question-form">
                                        <form id="addQuestionForm" method="post"
                                            action="controller.jsp?page=questions&operation=addnew"
                                            ...
                                            <div class="form-grid">
                                                <div class="form-group">
                                                    ...
                                                    <select name="coursename" class="form-select"
                                                        id="courseSelectAddNew" required onchange="saveLastSelection()">
                                                        <% if (courseNames.isEmpty()) { %>
                                                            <option value="">No courses available</option>
                                                            <% } else { %>
                                                                <option value="">Select Course</option>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <select id="questionTypeSelect" class="form-select"
                                                        onchange="toggleOptions(); saveLastSelection()">
                                                        <option value="MCQ" <%="MCQ" .equals(lastQuestionType)
                                                            ? "selected" : "" %>>Multiple Choice</option>
                                                        <option value="TrueFalse" <%="TrueFalse"
                                                            .equals(lastQuestionType) ? "selected" : "" %>>True/False
                                                        </option>
                                                        <option value="MultipleSelect" <%="MultipleSelect"
                                                            .equals(lastQuestionType) ? "selected" : "" %>>Multiple
                                                            Select (2 correct)</option>
                                                        <option value="FillInTheBlank" <%="FillInTheBlank"
                                                            .equals(lastQuestionType) ? "selected" : "" %>>Fill in the
                                                            Blank</option>
                                                        <option value="Code" <%="Code" .equals(lastQuestionType)
                                                            ? "selected" : "" %>>Code Snippet</option>
                                                        <option value="DragAndDrop" <%="DragAndDrop"
                                                            .equals(lastQuestionType) ? "selected" : "" %>>Drag and Drop
                                                        </option>
                                                    </select>
                                                    <input type="hidden" id="questionTypeHidden" name="questionType"
                                                        value="<%=lastQuestionType%>">
                                                </div>
                                            </div>

                                            <div class="form-group">
                                                <label class="form-label"><i class="fas fa-question-circle"
                                                        style="color: var(--primary-blue);"></i> Your Question</label>
                                                <textarea name="question" id="questionTextarea" class="question-input"
                                                    rows="3" oninput="handleQuestionInput()"
                                                    onpaste="handlePaste(event)"></textarea>
                                                <small class="form-hint">Enter your question text (optional if uploading
                                                    an image)</small>
                                            </div>

                                            <!-- Image Upload Section -->
                                            <div class="form-group">
                                                <label class="form-label"><i class="fas fa-image"
                                                        style="color: var(--info);"></i> Upload Question Image
                                                    (Optional)</label>
                                                <div class="drop-zone" id="imageDropZone">
                                                    <div class="drop-zone-content">
                                                        <i class="fas fa-cloud-upload-alt drop-icon"></i>
                                                        <p class="drop-text">Drag & drop your image here or click to
                                                            browse</p>
                                                        <p class="drop-hint">Supports JPG, PNG, GIF, WebP (Max 3MB)</p>
                                                        <input type="file" name="imageFile" class="form-control"
                                                            id="imageFile" accept=".jpg,.jpeg,.png,.gif,.webp"
                                                            style="display: none;">
                                                    </div>
                                                </div>
                                                <div id="imageFileNameDisplay" class="file-name-display"
                                                    style="display: none; margin-top: 10px;">
                                                    <i class="fas fa-image"></i>
                                                    <span id="imageFileName"></span>
                                                    <button type="button" class="remove-file-btn"
                                                        onclick="removeImageFile()">×</button>
                                                </div>
                                                <small class="form-hint">Upload an image to accompany your question
                                                    (optional)</small>
                                            </div>

                                            <!-- Image Preview Section -->
                                            <div id="imagePreviewSection" class="form-group" style="display: none;">
                                                <label class="form-label"><i class="fas fa-eye"
                                                        style="color: var(--success);"></i> Image Preview</label>
                                                <div
                                                    style="text-align: center; padding: 10px; border: 1px solid var(--medium-gray); border-radius: var(--radius-sm);">
                                                    <img id="imagePreview" src="#" alt="Image Preview"
                                                        style="max-width: 100%; max-height: 200px; display: none; border-radius: var(--radius-sm);">
                                                    <p id="previewPlaceholder"
                                                        style="color: var(--dark-gray); margin: 0;">Uploaded image will
                                                        appear here</p>
                                                </div>
                                            </div>
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <div class="options-grid">
                                                        <div class="option-container">
                                                            <textarea name="opt1" class="option-input"
                                                                placeholder="First Option" id="opt1" required
                                                                rows="2"></textarea>
                                                            <div class="error-message" id="opt1Error">First option is
                                                                required</div>
                                                        </div>
                                                        <div class="option-container">
                                                            <textarea name="opt2" class="option-input"
                                                                placeholder="Second Option" id="opt2" required
                                                                rows="2"></textarea>
                                                            <div class="error-message" id="opt2Error">Second option is
                                                                required</div>
                                                        </div>
                                                        <div class="option-container">
                                                            <textarea name="opt3" class="option-input"
                                                                placeholder="Third Option" id="opt3"
                                                                rows="2"></textarea>
                                                            <div class="error-message" id="opt3Error">Third option is
                                                                required</div>
                                                        </div>
                                                        <div class="option-container">
                                                            <textarea name="opt4" class="option-input"
                                                                placeholder="Fourth Option" id="opt4"
                                                                rows="2"></textarea>
                                                            <div class="error-message" id="opt4Error">Fourth option is
                                                                required</div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div id="dragDropContainer" style="display:none; margin-top: 20px;">
                                                <div class="alert"
                                                    style="background: #e3f2fd; color: #0c5460; border-color: #bee5eb;">
                                                    <i class="fas fa-info-circle"></i>
                                                    <strong>How to use:</strong> Upload a main image above (required).
                                                    Then add draggable items and define drop zones.
                                                </div>

                                                <div style="display: flex; gap: 20px; flex-wrap: wrap;">
                                                    <div style="flex: 1; min-width: 300px;">
                                                        <h5
                                                            style="color: var(--primary-blue); border-bottom: 2px solid var(--accent-blue); padding-bottom: 5px; margin-bottom: 15px;">
                                                            <i class="fas fa-arrows-alt"></i> Draggable Items
                                                        </h5>
                                                        <div id="draggableItemsList"></div>
                                                    ...
                                                    <div style="flex: 1; min-width: 300px;">
                                                        <h5
                                                            style="color: var(--primary-blue); border-bottom: 2px solid var(--accent-blue); padding-bottom: 5px; margin-bottom: 15px;">
                                                            <i class="fas fa-bullseye"></i> Drop Zones
                                                        </h5>
                                                ...
                                                <input type="hidden" name="draggableItemsJson" id="draggableItemsJson">
                                                <input type="hidden" name="dropZonesJson" id="dropZonesJson">
                                            </div>

                                            <div class="form-group">
                                                <label class="form-label"><i class="fas fa-check-circle"
                                                        style="color: var(--success);"></i> Correct Answer</label>
                                                ...
                                                <div id="trueFalseContainer" style="display:none;">
                                                    ...
                                                    <small class="form-hint">Select whether the statement is True or
                                                        False</small>
                                                </div>

                                                <!-- Regular Correct Answer Textarea -->
                                                <div id="correctAnswerContainer">
                                                    <textarea id="correctAnswer" name="correct" class="form-control"
                                                        required rows="2"
                                                        placeholder="Enter the correct answer"></textarea>
                                                    <small class="form-hint">Must match one of the options
                                                        exactly</small>
                                                </div>
                                                ...
                                                <div id="multipleCorrectContainer" style="display:none;">
                                                    <div class="options-grid">
                                                        <div class="form-check">
                                                            <input type="checkbox" id="correctOpt1"
                                                                class="form-check-input correct-checkbox">
                                                            <label for="correctOpt1" class="form-check-label">Option
                                                                1</label>
                                                        </div>
                                                        <div class="form-check">
                                                            <input type="checkbox" id="correctOpt2"
                                                                class="form-check-input correct-checkbox">
                                                            <label for="correctOpt2" class="form-check-label">Option
                                                                2</label>
                                                        </div>
                                                        <div class="form-check">
                                                            <input type="checkbox" id="correctOpt3"
                                                                class="form-check-input correct-checkbox">
                                                            <label for="correctOpt3" class="form-check-label">Option
                                                                3</label>
                                                        </div>
                                                        <div class="form-check">
                                                            <input type="checkbox" id="correctOpt4"
                                                                class="form-check-input correct-checkbox">
                                                            <label for="correctOpt4" class="form-check-label">Option
                                                                4</label>
                                                        </div>
                                                    </div>
                                            ...
                                            <div id="aiNavigation" class="ai-navigation"
                                                style="display: none; margin-top: 20px; padding: 15px; background: #f0f7ff; border: 1px solid #cce3ff; border-radius: 8px;">
                                                <div
                                                    style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                                                    <span id="aiCount"
                                                        style="font-weight: 600; color: #0056b3;">Reviewing AI Question
                                                        1 of 1</span>
                                                    <div style="display: flex; gap: 10px;">
                                                        <button type="button" class="btn btn-outline btn-sm"
                                                            onclick="showPreviousAIQuestion()" id="prevAIBtn">
                                                            <i class="fas fa-chevron-left"></i> Previous
                                                        </button>
                                                        <button type="button" class="btn btn-outline btn-sm"
                                                            onclick="showNextAIQuestion()" id="nextAIBtn">
                                                            Next <i class="fas fa-chevron-right"></i>
                                                        </button>
                                                        <button type="button" class="btn btn-success btn-sm"
                                                            onclick="saveAllAIQuestions()" id="saveAllAIBtn"
                                                            style="margin-left: 10px;">
                                                            <i class="fas fa-save"></i> Save All
                                                        </button>
                                                    </div>
                                                </div>
                                                <small style="display: block; color: #666;">Review and refine the
                                                    questions as needed. Click 'Add Question' to save.</small>
                                            </div>

                                            <div class="form-actions">
                                                <button type="reset" class="btn btn-outline"
                                                    onclick="currentAIQuestionIndex = -1; resetForm()">
                                                    <i class="fas fa-redo"></i>
                                                    Reset
                                                </button>
                                        ...
                                        <form action="" method="get" id="viewQuestionsForm">
                                            ...
                                            <div class="form-grid">
                                                <div class="form-group">
                                                    ...
                                                    <select name="coursename" class="form-select" id="courseSelectView"
                                                        onchange="updateShowAllButton()">
                                                        <option value="">All Courses</option>
                                                        <% if (courseNames !=null) { for (String course : courseNames) {
                                                            boolean isSelected=(lastCourseName !=null &&
                                                            lastCourseName.equals(course)); %>
                                            ...
                                            <div class="form-actions">
                                                <button type="button" class="btn btn-success" id="showAllQuestionsBtn"
                                                    onclick="showAllQuestions()" <%=courseNames.isEmpty() ? "disabled"
                                                    : "" %>>
                        ...
                        <script>
                            ...
                            function toggleOptions() {
                                ...
                                if (qType === "TrueFalse") {
                                    ...
                                    if (dragDrop) dragDrop.style.display = "block";
                            ...
                            function addDraggableItem() {
                                const id = Date.now();
                                const itemHtml = `
                                    <div class="draggable-item-row" id="item-${id}" style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; border-radius: 4px; background: #fff;">
                                        <div style="display: flex; gap: 10px; align-items: center;">
                                            <input type="text" class="form-control" placeholder="Item Label/Text" onchange="updateDraggableItem(${id}, this.value)">
                                            <button type="button" class="btn btn-outline-danger btn-sm" onclick="removeDraggableItem(${id})"><i class="fas fa-trash"></i></button>
                                        </div>
                                    </div>
                                `;
                                document.getElementById('draggableItemsList').insertAdjacentHTML('beforeend', itemHtml);
                                draggableItems.push({ id: id, label: '', image: '' });
                                updateHiddenInputs();
                            }

                            function removeDraggableItem(id) {
                                document.getElementById(`item-${id}`).remove();
                                draggableItems = draggableItems.filter(item => item.id !== id);
                                updateHiddenInputs();
                                updateDropZoneSelects();
                            }

                            function updateDraggableItem(id, label) {
                            ...
                            function addDropZone() {
                                const id = Date.now();
                                const options = draggableItems.map(item => `<option value="${item.id}">${item.label || 'Unnamed Item'}</option>`).join('');

                                const zoneHtml = `
                                    <div class="drop-zone-row" id="zone-${id}" style="border: 1px solid #ddd; padding: 10px; margin-bottom: 10px; border-radius: 4px; background: #fff;">
                                        <div style="display: flex; gap: 10px; align-items: center; margin-bottom: 5px;">
                                            <input type="text" class="form-control" placeholder="Zone Label (e.g. Zone 1)" onchange="updateDropZone(${id}, 'label', this.value)">
                                            <button type="button" class="btn btn-outline-danger btn-sm" onclick="removeDropZone(${id})"><i class="fas fa-trash"></i></button>
                                        </div>
                                        <select class="form-select" onchange="updateDropZone(${id}, 'correctItem', this.value)">
                                            <option value="">Select Correct Item</option>
                                            ${options}
                                        </select>
                                    </div>
                                `;
                                document.getElementById('dropZonesList').insertAdjacentHTML('beforeend', zoneHtml);
                                dropZones.push({ id: id, label: '', correctItem: '' });
                                updateHiddenInputs();
                            }
                            ...
                            function updateDropZone(id, field, value) {
                            ...
                            function updateDropZoneSelects() {
                                const selects = document.querySelectorAll('#dropZonesList select');
                                selects.forEach(select => {
                                    const currentVal = select.value;
                                    const options = draggableItems.map(item => `<option value="${item.id}" ${item.id == currentVal ? 'selected' : ''}>${item.label || 'Unnamed Item'}</option>`).join('');
                                    select.innerHTML = '<option value="">Select Correct Item</option>' + options;
                                });
                            }

                            function updateHiddenInputs() {
                                document.getElementById('draggableItemsJson').value = JSON.stringify(draggableItems);
                                document.getElementById('dropZonesJson').value = JSON.stringify(dropZones);
                            }
