    ...
    <main class="content-area">
        ...
        %>
            ...
            <script>
                ...
                /* --- DRAG AND DROP FUNCTIONALITY - PHASE 4 Implementation --- */
                let userMappings = {}; // Global mappings object
                ...
                function renderDragDropInterface(qIdx, dragContainer, dropContainer, items, targets) {
                    ...
                    targets.forEach(target => {
                        ...
                        el.innerHTML = `
                            <div class="drop-target-header">${target.label}</div>
                ...
                function handleDrop(e) {
                    ...
                    const qIdx = target.id.split('_')[0].substring(1);
                    const targetId = target.dataset.targetId;
                    const itemDataId = draggedEl.dataset.itemId;
                    const itemText = draggedEl.dataset.text;

                    // Check if target already has an item
                    if (target.querySelector('.dropped-item')) {
                        // Return existing item to pool
                        const existingDropped = target.querySelector('.dropped-item');
                        restoreItemToPool(qIdx, existingDropped.dataset.itemId, existingDropped.dataset.text);
                    ...
                    droppedEl.dataset.itemId = itemDataId;
                    droppedEl.dataset.text = itemText;
                    droppedEl.innerHTML = `
                        <span>${itemText}</span>
                    ...
                    target.querySelector('.placeholder')?.remove();

                    // Hide original item from pool
                ...
                function restoreItemToPool(qIdx, itemId, text) {
                    const pool = document.getElementById(`dragItems_${qIdx}`);
                    const originalItem = document.getElementById(`q${qIdx}_item_${itemId}`);
                    if (originalItem) {
                ...
                function removeItemFromTarget(qIdx, targetId, itemId, text) {
                    const target = document.getElementById(`q${qIdx}_target_${targetId}`);
                    if (target) {
                ...
                function saveDragDropAnswer(qIdx) {
                    const mappings = userMappings[qIdx];
                    // Create a JSON string of matches: { targetId: itemId }
                    // Format matching what validateDragDropAnswer expects: { "target_1": "item_1", ... }
                    const formattedMappings = {};
                    for (let tId in mappings) {
                        formattedMappings[`target_${tId}`] = `item_${mappings[tId]}`;
                    }
                    const answer = JSON.stringify(formattedMappings);
